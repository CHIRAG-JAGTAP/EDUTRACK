...
function mimeToOCRFiletype(mimeType) {
...
function handleTTFile(file) {
    ...
    fileToBase64(file).then(({ base64, mimeType, dataUrl }) => { S.tt.base64 = base64; S.tt.mimeType = mimeType; S.tt.dataUrl = dataUrl; });
}
function handleCalFile(file) {
    ...
    reader.readAsDataURL(file);
    fileToBase64(file).then(({ base64, mimeType, dataUrl }) => { S.cal.base64 = base64; S.cal.mimeType = mimeType; S.cal.dataUrl = dataUrl; });
}
...
async function analyzeTimetable() {
    ...
    try {
        const mimeType = S.tt.mimeType || 'image/jpeg';
        const filetype = mimeToOCRFiletype(mimeType);
        const formData = new FormData();
        formData.append('base64Image', `data:${mimeType};base64,${S.tt.base64}`);
        ...
        if (data.OCRExitCode !== 1) throw new Error(data.ErrorMessage?.[0] || data.ErrorMessage || 'OCR failed. Try a clearer image.');
...
async function analyzeCalendar() {
    ...
    loading.style.display = 'flex'; wrap.style.display = 'none';
    try {
        const mimeType = S.cal.mimeType || 'image/jpeg';
        const filetype = mimeToOCRFiletype(mimeType);
        const formData = new FormData();
        formData.append('base64Image', `data:${mimeType};base64,${S.cal.base64}`);
        formData.append('apikey', OCR_API_KEY);
        formData.append('isOverlayRequired', 'false');
        formData.append('filetype', filetype);
        formData.append('scale', 'true');
...
function parseSubjectsFromText(text) {
    ...
    // Only skip lines that are clearly NOT subject names: pure numbers, single chars, time patterns, or header junk
    const skipPatterns = [
        /^\d+[\d:.\-\/]*$/, // pure numbers / times like 9:00, 10-11
        /^(mon|tue|wed|thu|fri|sat|sun|monday|tuesday|wednesday|thursday|friday|saturday|sunday)$/i,
        /^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)/i,
        /^\s*$/, // blank
        /^[^a-zA-Z]{0,2}$/, // less than 3 alpha chars
    ];
    const seen = new Set();
    const subjects = [];
    for (const line of lines) {
        if (skipPatterns.some(p => p.test(line))) continue;
        // Must contain at least 3 consecutive letters (a real word)
        if (!/[A-Za-z]{3}/.test(line)) continue;
        // Deduplicate case-insensitively
        const key = line.toLowerCase().replace(/\s+/g, ' ');
        if (seen.has(key)) continue;
        seen.add(key);
        subjects.push({ name: line, type: 'subject' });
    }
    return subjects;
}
...
function restoreTTResults() {
    if (S.tt.base64) { document.getElementById('tt-preview-img').src = S.tt.dataUrl || `data:image/jpeg;base64,${S.tt.base64}`; document.getElementById('tt-preview-wrap').style.display = 'block'; document.getElementById('tt-drop-zone').style.display = 'none'; }
    if (S.cal.base64) { document.getElementById('cal-preview-img').src = S.cal.dataUrl || `data:image/jpeg;base64,${S.cal.base64}`; document.getElementById('cal-preview-wrap').style.display = 'block'; document.getElementById('cal-drop-zone').style.display = 'none'; }
}
...
async function logoutUser() {
    ...
    } catch (e) { /* ignore – session is flushed server-side */ }
    // Replace current history entry so the back button cannot return to dashboard
